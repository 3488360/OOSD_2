...
public class Game {
	...
	public Game() {
		// String p1 = (String) JOptionPane.showInputDialog(null,
		// "Please enter player 1's name:", "Player 1 Name", 3);
		// String p2 = (String) JOptionPane.showInputDialog(null,
		// "Please enter player 2's name:", "Player 2 Name", 3);
		String p1 = "john";
		String p2 = "adi";
	...
	private boolean selectAndPlace(Player p) {
		...
		while (!isReady) {
			...
			if (selectedPiece != null) {
				userInterface.updateSelectedPiece(selectedPiece, null);

				switch (selectedPiece) {
				case "King":
					piece = new KingPiece(p);
					break;

				case "Queen":
					piece = new QueenPiece(p);
					break;
				...
				case "Default Layout":
					board.setPiece(new Coordinate(7, 14),
							new TestPiece(player1));
					board.setPiece(new Coordinate(6, 14),
							new KingPiece(player1));
					board.setPiece(new Coordinate(5, 14),
							new KingPiece(player1));
					board.setPiece(new Coordinate(8, 14),
							new KingPiece(player1));
					board.setPiece(new Coordinate(9, 14),
							new KingPiece(player1));
				...
				if (piece != null) {
					...
					System.out.println("Placing " + piece.getName() + " at "
	...
	public void passCoordinates(Coordinate coor) {
		...
		if (currentlySelected == null) {
			...
			if (board.getPiece(currentlySelected) != null
					&& turn == board.getPiece(currentlySelected).getPlayer()) {
				gameController.updateSelectedPiece(board.getPiece(
						currentlySelected).getName(), board.getPiece(currentlySelected).getMoves(
								currentlySelected));
			}
...
//			turnAllMoveableSquaresOff();
	...
	private void DisplayALlMoveableSquare() {
		// display all the places the piece can possibly move to
		List<Coordinate> co = board.getPiece(currentlySelected).getMoves(
				currentlySelected);
		Cell[][] cell = board.getAllCells();
		for (int i = 0; i < co.size(); i++) {
			if (co.get(i).x < board.getWidth() && co.get(i).x >= 0
					&& co.get(i).y < board.getHeight() && co.get(i).y >= 0) {
	...
	private void calculateMove() {
		...
		if (board.getPiece(currentlySelected) != null) {
			if (board.getPiece(destinationSelected) != null) {
				...
				if (board.getPiece(destinationSelected).getPlayer()
						.equals(turn)) {
					userInterface
							.message("You are trying to attack your own piece!");
				} else {
					int attack;
					int health;
					attack = board.getPiece(currentlySelected).getStrength();
					health = board.getPiece(destinationSelected)
							.getCurrentHealth();
				...
				if (turn.equals(board.getPlayer(currentlySelected))) {
					...
					// moves =
					// board.getMovement(board.getPiece(currentlySelected),
					// currentlySelected);
					board.setPiece(destinationSelected,board.getPiece(currentlySelected));
					board.setPiece(currentlySelected, null);
					userInterface.updateBoard();
					...
					System.out.println("That is not " + turn.getName()
							+ "'s piece!");
			}
		} else {
